...
original_discrete::original_discrete
                  ( double x_inital_, double alpha_x_,double alpha_y_, 
                    double dt_, const char * pattern_, 
                    double tau_, int num_gbfs_, 
                    std::vector<double> y_demo_, double goal_,
                    std::vector<double> obstacle_)
                : CanonicalSystem(x_inital_, alpha_x_, dt_, pattern_, tau_, y_demo_), 
                  num_gbfs(num_gbfs_), 
                  alpha_y(alpha_y_), 
                  beta_y(alpha_y_ / 4),
                  y_demo(y_demo_), 
...
/*
   ...
   So hi = num_gbfs /  gbfs_center_point[d]
*/
std::vector<std::vector<double>> original_discrete::generate_gbfs_Psi()
{
    // Set the centre of the Gaussian basis functions be spaced evenly throughout run time
    // 生成基础高斯函数的中心点, 根据 run_time 均匀分布
    // std::vector<double> gbfs_center_point(num_gbfs) ; 
    std::vector<double> gbfs_variance(num_gbfs) ;
    
    std::vector<double> discrete_x_track = generate_CS_track() ; // return x_track , number : time_steps
    // get_time_steps() = run().size() = run_track.size
    int discrete_time_steps = get_time_steps() ;  
    // 构造容器，装载数组
    std::vector<std::vector<double>> discrete_Psi(num_gbfs);
    std::vector<double> t_center(num_gbfs);
    std::vector<double> t_track(discrete_time_steps);
    std::vector<double> Psi_center(num_gbfs) ; 
    double distance = get_run_time() / num_gbfs ;

    /* first method : come from 沙漏 */
    // for(int h = 0; h < num_gbfs; ++h)
    // {
    ...
    // it is not time_steps / num_gbfs
    double discrete_tau = get_tau();
    double discrete_alpha_x = get_alpha_x();
    for (int d = 0; d < num_gbfs; ++d )
    {
        std::vector<double> Psi_track(discrete_time_steps, 0) ;
        
        // x = exp(-tau * alpha_x * t)
        // 将run_time上的均匀分布，映射到 Canonical System 对应的 x 值上
        // gbfs_center_point[d] =  exp(- discrete_tau * discrete_alpha_x * Psi_center[d]) ;

        Psi_center[d] = exp(- discrete_tau * discrete_alpha_x * distance * d) ;
        gbfs_variance[d] = (std::pow(num_gbfs, 1.5) / Psi_center[d]) / discrete_alpha_x ;
...
std::vector<double> original_discrete::generate_derivation(std::vector<double> path)
{
...
std::vector<double> original_discrete::generate_f_target()
{
    double tau = get_tau() ;
    int t = get_time_steps() ;
    
    std::vector<double> dot_y_demo(t, 0) ; 
    std::vector<double> ddot_y_demo(t, 0) ;
    std::vector<double> f_target(t, 0) ;

    dot_y_demo = generate_derivation(y_demo) ; 
    ddot_y_demo = generate_derivation(dot_y_demo) ; 
    for (int g= 0; g < t; ++g)
    {
        f_target[g] = tau*tau*ddot_y_demo[g] - alpha_y*(beta_y*(goal - y_demo[g]) - tau*dot_y_demo[g]);
    }
...
// 空间缩放项
double original_discrete::generate_scaling_term()
{
    double scaling_term = goal - y_inital ; 
    return scaling_term ;
}
...
/*
   ...
   s : t x 1 ; gamma t x t ; f_target
    ...
    s = 缩放项 ； gamma = Psi ;  
*/
std::vector<double> original_discrete::generate_weights()
{
    std::vector<std::vector<double>> Psi = generate_gbfs_Psi();
    std::vector<double> cs_track = generate_CS_track() ;
    int t = get_time_steps() ; 

    std::vector<double> f_target = generate_f_target();
    std::vector<double> wi(num_gbfs); // 与高斯数量有关
    double scaling_item = generate_scaling_term();


    for (int m = 0; m < num_gbfs; ++m)
    {
        // 重置
        std::vector<double> molecule(num_gbfs, 0);  // 分子
        std::vector<double> denominator(num_gbfs, 0);  // 分母

        for (int n = 0; n < t; ++n)
        {
            molecule[m] += cs_track[n] * Psi[m][n] * f_target[n];
            // scaling_item[n] = goal - y_demo[n] ; 
...
std::vector<double> original_discrete::generate_force_term()
{
    int time = get_time_steps() ;
...
std::vector<double> original_discrete::generate_DMP_track()
{
    int time = get_time_steps() ;
    ...
    for (int u = 0; u < time ; ++u)
    {
        ...
        if( u == 0 )
        {
            ddot_y_dmp[u] = alpha_y*(beta_y * (goal - y_dmp[u]) - dot_y_dmp[u]) + force[u] ; 
        ...
        else
        {
            ...
            {
                // 空数组 {} 即是 没有障碍物
                ddot_y_dmp[u] = alpha_y*(beta_y * (goal - y_dmp[u-1]) - dot_y_dmp[u-1]) + force[u] ;
            ...
            else
            {
                ...
                double beta = 20.0 / pi ; 
                double x = u * delta_t ; 
                double distance_x = obstacle[0] - x ; 
                double distance_y = obstacle[1] - y_dmp[u-1] ;
                ...
                if (difference < ref)
                {
                    ...
                    std::vector<double> obstacle_world = { rotation[0]*distance_x + rotation[1]*distance_y,
                                                           rotation[2]*distance_x + rotation[3]*distance_y };
                    double theta_obs = std::atan2(obstacle_world[1], obstacle_world[0]);
                    double dphi = gamma * theta_obs * exp(- beta * std::abs(theta_obs) );
                    // 求外积 a = [a1, a2]，b = [b1, b2] 2x1  ; np.outer(obstacle - y, dy)
                    //     a1*b1 a1*b2
                    //     a2*b1 a2*b2
                    ...
                    p += pval[1];
                    ddot_y_dmp[u] = alpha_y*(beta_y * (goal - y_dmp[u-1]) - dot_y_dmp[u-1]) + force[u] + p ;
                } 
                else
                {
                    ddot_y_dmp[u] = alpha_y*(beta_y * (goal - y_dmp[u-1]) - dot_y_dmp[u-1]) + force[u] ;
