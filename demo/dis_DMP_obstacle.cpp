...
int main()
{
    ...
    int gen_obs_steps = length_y_demo * (2.0/3)  ;
    ...
    std::vector<double> obstacle= { gen_obs_steps*dt, y_demo[gen_obs_steps]} ; // 空数组 
    std::vector<double> obstacle_x = {obstacle[0]} ; 
    std::vector<double> obstacle_y = {obstacle[1]} ;
    ...
    std::map<std::string, std::string> keywords = {{"marker", "*"}} ; 
    plt::scatter(obstacle_x, obstacle_y, 50, keywords);
